...
template "/root/.my.cnf" do
  variables(:root_password => node["mysql"]["root_password"])
  owner "root"
  group "root"
  mode 0600
...
execute "Update MySQL root password" do
  mysql_pwd = node["mysql"]["root_password"]
  command "mysqladmin --user=root --password='' password '#{mysql_pwd}'"
...
template "/etc/mysql/debian.cnf" do
  ...
  variables(:debian_password => node["mysql"]["debian_user_password"])
  owner "root"
  group "root"
  mode 0640
  notifies :restart, "service[mysql]", :immediately if node["percona"]["auto_restart"]
...
if firstnode
    ...
    execute "add-mysql-user-sstuser" do
        command "/usr/bin/mysql -u root -p#{mysql_pwd} -D mysql -r -B -N -e \"CREATE USER '#{sstAuthName}'@'localhost' IDENTIFIED BY '#{sstauthPass}'\""
        #command "/usr/bin/mysql -D mysql -r -B -N -e \"CREATE USER '#{sstAuthName}'@'localhost' IDENTIFIED BY '#{sstauthPass}'\""
        action :run
        #Chef::Log.info('****COE-LOG add-mysql-user-sstuser')
        #only_if { `/usr/bin/mysql -u root -p#{mysql_pwd} -D mysql -r -B -N -e \"SELECT COUNT(*) FROM user where User='sstuser' and Host = 'localhost'"`.to_i == 0 }
        only_if { `/usr/bin/mysql -D mysql -r -B -N -e \"SELECT COUNT(*) FROM user where User='sstuser' and Host = 'localhost'"`.to_i == 0 }
    end
    # Grant priviledges
    execute "grant-priviledges-to-sstuser" do
        #Chef::Log.info('****COE-LOG grant-priviledges-to-sstuser')
        command "/usr/bin/mysql -u root -p#{mysql_pwd} -D mysql -r -B -N -e \"GRANT RELOAD, LOCK TABLES, REPLICATION CLIENT ON *.* TO '#{sstAuthName}'@'localhost'\""
        #command "/usr/bin/mysql -D mysql -r -B -N -e \"GRANT RELOAD, LOCK TABLES, REPLICATION CLIENT ON *.* TO '#{sstAuthName}'@'localhost'\""
        action :run
    ...
    execute "flush-mysql-priviledges" do
        ...
        command "/usr/bin/mysql -u root -p#{mysql_pwd} -D mysql -r -B -N -e \"FLUSH PRIVILEGES\""
        #command "/usr/bin/mysql -D mysql -r -B -N -e \"FLUSH PRIVILEGES\""
        action :run
    #DEL    only_if { `/usr/bin/mysql -u root -p#{mysql_password} -D mysql -r -B -N -e \"SELECT COUNT(*) FROM user where User='sstuser' and Host = 'localhost'"`.to_i == 0 }
    end
end
